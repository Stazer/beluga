...
{
    if(check_length && !this->has_minimum_length(5))
	return false;
...
{
    if(check_length && !this->has_minimum_length(6))
	return false;
...
{
    if(check_length && !this->has_minimum_length(35))
	return false;
...
{
    if(check_length && !this->has_minimum_length(1))
	return false;
    ...
    if(check_length && !this->has_minimum_length(session_id_length))
	return false;
...
{
    if(check_length && !this->has_minimum_length(2))
	return false;
    ...
    if(check_length && !this->has_minimum_length(cipher_suites_length))
	return false;
...
{ 
    ...
    if(check_length && !this->has_minimum_length(compression_methods_length))
	return false;
...
bool beluga::tls_reader<iterator_type>::read_extensions(tls_client_hello::extensions_type& extensions, bool check_length)
{
    ...
    if(check_length && !this->has_minimum_length(extensions_length))
	return false;
    
    while(extensions_length > 0)
    {
	if(check_length && !this->has_minimum_length(4))
	    return false;
	
	std::uint16_t type = 0, length = 0;
	this->get_reader() >> type >> length;
	
	if(check_length && !this->has_minimum_length(length))
	    return false;
