...
bool beluga::http_reader<iterator_type>::read_request_line(http_method& method, std::string& uri, http_version& version)
{
    ...
    auto iterator = std::search(this->get_iterator(), this->get_to(), delimiter.begin(), delimiter.end());    

    if(iterator == this->get_to())
	return false;
    
    std::string content = std::string(this->get_iterator(), iterator);
    ...
    this->set_iterator(iterator + delimiter.length());
